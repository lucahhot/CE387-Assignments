...
*/
    ...
    float sum_sin_diff = 0;
    float sum_cos_diff = 0;
    for ( int i = -360; i <= 360; i++ )
    {
        float p = i * M_PI / 180;        
        int p_fixed = QUANTIZE_F(p);
        short s = 0, c = 0;
        // printf("Real angle: %d, Quantized angle: %d\n", p, p_fixed);
        ...
        sum_sin_diff += abs(DEQUANTIZE_F(s) - sin(p));
        sum_cos_diff += abs(DEQUANTIZE_F(c) - cos(p));
    ...
    printf("Total sum of sin error = %8.16f\n", sum_sin_diff);
    printf("Total sum of cos error = %8.16f\n", sum_cos_diff);

    printf("Percent difference of sin error: %8.8f\n", abs(sum_sin_diff-0.1116878507236145)/sum_sin_diff);
    printf("Percent difference of cos error: %8.8f\n", abs(sum_cos_diff-0.1149921312188339)/sum_cos_diff);
